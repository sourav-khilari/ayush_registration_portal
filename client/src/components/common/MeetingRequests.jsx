...
export default function MeetingRequests() {
  ...
  return (
    <div className="p-4">
      <h2 className="text-xl font-bold mb-3 text-gray-900 dark:text-gray-100">Meeting Requests</h2>

      {requests.length === 0 && <p className="text-gray-500 dark:text-gray-400">No requests</p>}

      {requests.map((req) => (
        <div
          key={req._id}
          className="border border-gray-200 dark:border-gray-700 p-3 rounded mb-2 flex justify-between bg-white dark:bg-gray-900"
        >
          <div>
            <div className="font-semibold text-gray-900 dark:text-gray-100">{req.senderId?.name || "Investor"}</div>
